...
def make_toc_item(global_config, metadata, root_path):
    ...
    return TOC_ITEM_TEMPLATE.format(
        get_printed_date(metadata), root_path + "/" + link, metadata["title"]
    )
...
if __name__ == "__main__":
    ...
    if "--sync" in sys.argv:
        os.system(
            "rsync -av site/. {}:{}".format(
                global_config["server"], global_config["website_root"]
            )
        )
        sys.exit()
    ...
    for file_location in sys.argv[1:]:
        ...
        total_file_contents = (
            ...
            + TITLE_TEMPLATE.format(
                metadata["title"], get_printed_date(metadata), root_path
            )
    ...
    for filename in os.listdir("posts"):
        if filename[-4:-1] != ".sw":
            metadatas.append(
                extract_metadata(open(os.path.join("posts", filename)), filename)
            )
    ...
    homepage_toc_items = [
        ...
        if global_config.get("homepage_category", "")
